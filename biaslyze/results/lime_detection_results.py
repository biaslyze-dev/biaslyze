...
class LimeDetectionResult:
    ...
    def dashboard(self, use_position=False):
        ...
        res_df = pd.DataFrame(
            {
                ...
                "bias_concepts": [
                    sample.bias_concepts for sample in self.biased_samples
                ],
                "keyword_position": [
                    sample.keyword_position for sample in self.biased_samples
                ],
                "bias_keywords": [
                    sample.bias_reasons for sample in self.biased_samples
                ],
        ...
        res_df["bias_concepts_joined"] = res_df.bias_concepts.apply(
            lambda x: ", ".join(x)
        )
        res_df["bias_keywords_joined"] = res_df.bias_keywords.apply(
            lambda x: ", ".join(x)
        )
        ...
        def bkapp(doc):
            ...
            columns = [
                ...
                TableColumn(
                    field="bias_concepts_joined", title="bias_concepts", width=100
                ),
                TableColumn(
